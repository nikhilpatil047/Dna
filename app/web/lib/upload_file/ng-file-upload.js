...
app.version = '5.0.9';
app.service('Upload', ['$http', '$q', '$timeout', function ($http, $q, $timeout) {
...
(function () {
    app.directive('ngfSelect', ['$parse', '$timeout', '$compile',
...
//                }
    ...
    app.validate = function (scope, $parse, attr, file, evt) {
    ...
    app.updateModel = function ($parse, $timeout, scope, ngModel, attr, fileChange,
    ...
    var validate = app.validate;
    var updateModel = app.updateModel;
...
(function () {
  ...
  app.directive('ngfDrop', ['$parse', '$timeout', '$location', function ($parse, $timeout, $location) {
    return {
      restrict: 'AEC',
      require: '?ngModel',
      link: function (scope, elem, attr, ngModel) {
  ...
  app.directive('ngfNoFileDrop', function () {
    return function (scope, elem) {
      if (dropAvailable()) elem.css('display', 'none');
    };
  });

  app.directive('ngfDropAvailable', ['$parse', '$timeout', function ($parse, $timeout) {
  ...
  app.directive('ngfSrc', ['$parse', '$timeout', function ($parse, $timeout) {
    return {
      restrict: 'AE',
